...
YDB.Terminal = {
    ...
    execute: function (cmd) {
        ...
        if (lower === 'help' || lower === '/help') {
            output.innerHTML += '<div class="text-base-content/80 whitespace-pre font-mono text-xs leading-relaxed">'
                + '\n'
                + '  ┌─────────────────────────────────────────────────────┐\n'
                ...
                + '  │  <span class="text-success">/status</span>            Connection status          │\n'
                + '  │  <span class="text-success">/connections</span>       List all connections       │\n'
                + '  │  <span class="text-success">/schema</span>            Show current DB schema     │\n'
                + '  │  <span class="text-success">/tables</span>            List tables                │\n'
                + '  │  <span class="text-success">/desc</span> [table]      Describe table columns     │\n'
                + '  │  <span class="text-success">/use</span> [name]        Switch connection          │\n'
                + '  │  <span class="text-success">/ask</span> [question]    Ask Copilot                │\n'
                + '  │  <span class="text-success">/export csv</span>        Export last result          │\n'
                + '  │  <span class="text-success">/clear</span>             Clear terminal             │\n'
                + '  │  <span class="text-success">/history</span>           Command history            │\n'
                + '  │                                                     │\n'
                + '  │  SQL:                                               │\n'
                + '  │  <span class="text-info">SELECT * FROM users LIMIT 10</span>                 │\n'
                + '  │  <span class="text-info">SELECT COUNT(*) FROM orders</span>                  │\n'
                + '  │                                                     │\n'
                + '  │  <span class="text-warning">Arrow Up/Down</span>  History  <span class="text-warning">Enter</span>  Execute      │\n'
                + '  └─────────────────────────────────────────────────────┘\n'
                + '</div>';
